...
## 11. 최종 통합본(2026-09-14)

이 폴더는 사용자가 작업한 `feature/client-remaining-menus`의 관리 기능과, `fix/final-hardening`의 인증·세션·저장 안정화 변경을 통합한 로컬 최종본이다. 원격 `codex/safetyvision-inference-fixes`의 추론 개선안도 비교했지만, 해당 브랜치는 외부 Person/Mask 모델 파일이 저장소에 없고 기본 설정(896 입력)과 현재 확보된 PPE 모델(640 입력)이 달라 그대로는 재현 가능한 최종본으로 채택하지 않았다. 자세한 근거는 `docs/버전비교_및_최종통합결론.md`에 기록했다.

최종 통합본에 포함된 보완 사항:

- 카메라·장비·사용자·시스템 설정 관리 메뉴와 서버 요청/응답을 연결하고, 관리 기능은 관리자 세션만 허용한다.
- 통계 기간 조회는 클라이언트의 현지 날짜를 UTC 시작 시각과 종료일 미포함 상한으로 변환해 날짜 경계 오류를 줄인다.
- 저장된 카메라 인덱스·해상도를 앱 시작 시 로드하고 현장 검사에 실제 반영하며, 카메라 화면 이탈 시 미리보기를 중지한다.
- `safetyvision_v2_640.onnx`의 경로를 실행 위치와 빌드 출력 위치에서 안전하게 찾고, 현재 검증된 640 PPE 모델의 다중 클래스 후처리와 Person 대체 경로를 유지한다.
- 로그인 전 요청 차단, 세션별 추론 제한, 프레임 순서 검증, 저장 재시도 중복 방지, DB 마이그레이션 및 카메라명 보완을 유지한다.

최종 확인 기록:

- 모델 SHA-256: `EA18AE903A566E8FA76F3EE1C503075522DCA269269315E9C862EFA170430B35`
- 전체 솔루션 빌드: 오류 0개
- MySQL 연결 통합 테스트: 48개 통과
- 신규 DB 마이그레이션·관리자 Seed·ONNX 로드·TCP 관리자 권한/CRUD/통계/설정/검사 세션 및 PPE 이미지 5장 전송: 통과
- WPF 창을 직접 띄우는 카메라 육안 검증은 현재 실행 환경 제약으로 미실시
